...
public class Messaging {
	...
	@Bean
	public Declarables userExternalCreatedQueueDeclarable(MessagingProperties messagingProperties) {
		return this.createEventQueueDeclarable(messagingProperties.getUserExternalCreated());
	}

	@Bean
	public TopicExchange userCreatedTopicExchange(MessagingProperties messagingProperties) {
		log.debug(
		...
		return new TopicExchange(messagingProperties.getUserCreated().getExchange());
	}

	@Bean
	public Declarables userExternalUpdatedQueueDeclarable(MessagingProperties messagingProperties) {
		return this.createEventQueueDeclarable(messagingProperties.getUserExternalUpdated());
	}

	@Bean
	public TopicExchange userUpdatedTopicExchange(MessagingProperties messagingProperties) {
		log.debug(
			"Create {} topic exchange with {} routing-key",
		...
		return new TopicExchange(messagingProperties.getUserUpdated().getExchange());
	}
